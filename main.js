...
const bibliotecaPersistenciaLivro = require('./persistencia_livro/biblioteca_persistencia')
const bibliotecaNegocioLivro = require('./negocio_livro/biblioteca_negocio');
...
//-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=
    ...
    try {
        const buscarStatusLivro1 = await bibliotecaNegocioLivro.buscarStatus('disponível')
    ...
    try {
        await bibliotecaPersistenciaLivro.retirarLivros(1, 1)
...
//-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=-=-==-==-=-==-=
    ...
    // Caso de sucesso para deletar por id (id existente)
    try{
        const clienteDeletado = await bibliotecaNegocioCliente.deletarCliente(3);
